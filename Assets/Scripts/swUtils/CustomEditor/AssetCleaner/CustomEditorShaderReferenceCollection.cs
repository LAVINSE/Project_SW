...
public class CustomEditorShaderReferenceCollection : IReferenceCollection
{
    // shader name / shader file guid
    public Dictionary<string, string> shaderFileList = new Dictionary<string, string>();
    private List<CustomEditorCollectionData> references = new List<CustomEditorCollectionData>();
    ...
    void CheckReference()
    {
        foreach (var shader in shaderFileList)
        {
            ...
            var code = CustomEditorClassReferenceCollection.StripComment(File.ReadAllText(shaderFilePath));
