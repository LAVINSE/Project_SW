...
public class CustomEditorAssetCollector
{
    public static readonly string exportXMLPath = "referencemap.xml";

    public List<string> deleteFileList = new List<string>();
    List<CustomEditorCollectionData> referenceCollection = new List<CustomEditorCollectionData>();
    ...
    public void Collection(string[] collectionFolders)
    {
        ...
        try
        {
            XmlSerializer serialize = new XmlSerializer(typeof(List<CustomEditorCollectionData>));
            ...
            if (File.Exists(exportXMLPath))
            {
                ...
                {
                    referenceCollection = (List<CustomEditorCollectionData>)serialize.Deserialize(fileStream);
                    fileStream.Close();
                }
            }
            ...
            if (useCodeStrip)
            {
                collectionList.Add(new CustomEditorClassReferenceCollection(saveEditorExtensions));
            }

            collectionList.AddRange(new IReferenceCollection[] {
                    new CustomEditorShaderReferenceCollection(),
                    new CustomEditorAssetReferenceCollection(),
                });
    ...
    void UnregistReferenceFromIgnoreList()
    {
        ...
        {
            var code = CustomEditorClassReferenceCollection.StripComment(File.ReadAllText(path));
